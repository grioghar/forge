...
printf '%s\n%s\n%s\n%s\n%s\n%s\n\n%s\n' "$OBJ" \
  "$REPO/forge/lumen_colors.sg" "$REPO/forge/wm_pal_macos.sg" "$REPO/forge/app/forge_gui.sg" \
  "$REPO/forge/app/anvil_dialog.sg" "$REPO/forge/app/library.sg" "macho" | "$CC0"
ld -o "$OUT" "$OBJ" -e _main -lSystem -framework Cocoa -framework QuartzCore -syslibroot "$SDK"; rm -f "$OBJ"
echo "Built: $OUT"
